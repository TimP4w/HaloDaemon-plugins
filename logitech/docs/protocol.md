...
Logitech's proprietary protocol layered on USB HID: this page documents the shared wire substrate (frame format, transport routing, request/response multiplexing, sector CRC) that both protocol generations ride on.
...
Two distinct generations coexist over one shared wire frame:
...
The generation-specific register and feature tables live in their own pages above. The current Lua implementation is [`main.lua`](../main.lua); historical Rust symbol names are retained below where they make the old protocol notes easier to follow.
...
The meaning of bytes 2-3 differs per generation: see [HID++ 1.0](hidpp1.md) (sub-ID encodes direction plus register bit 9) and [HID++ 2.0](hidpp2.md) (feature index plus function|software-id).

---

## 2. Functions

The substrate exposes transport plumbing, not device commands (those live in the generation pages).

### Short/long collections
...
The LIGHTSPEED headsets expose an interface that declares no short report: the 7-byte short form returns `EPIPE`. `open_wired` with `DirectReport::LongOnly` builds the messenger with `with_long_requests()`, routing every request (ROOT included) onto a long (`0x11`) report.
...
### Reader self-termination

A run of `MAX_CONSECUTIVE_READ_ERRORS` (20) read errors, or an explicit "disconnected"/"poll error", stops the listener (`classify_read_error`); this is just ahead of the 2 s hotplug monitor, so a vanished device is noticed promptly.
...
| --- | --- | --- |
| Short report ID | `0x10` | 7-byte frame, 3 param bytes |
| Long report ID | `0x11` | 20-byte frame, 16 param bytes |
| `RECEIVER_DEVNUM` | `0xFF` | Receiver itself, or a directly wired device |
| Paired device numbers | `1..=6` | Receiver pairing slots |
| Error sub-ID (wired) | `0x8F` | Byte 2 of an error reply |
| Error sub-ID (Lightspeed wireless) | `0xFF` | Byte 2 of an error reply |
| Request/write timeout | 2 s | Per request, write and reply wait each |
| `MAX_CONSECUTIVE_READ_ERRORS` | 20 | Reader self-termination threshold |
...
Unmatched packets become `HidppNotification { devnum, sub_id, address, data }` broadcasts, except the dropped orphan case above. HID++ 1.0 sub-IDs (`>= 0x40`) put a device/register byte in byte 3, not a swid, so they always broadcast. Connect/disconnect and pairing notifications are HID++ 1.0 broadcasts from the receiver; see [HID++ 1.0](hidpp1.md).

---

## Notes

- The register and feature semantics of bytes 2-3 are generation-specific; this page intentionally documents only the shared frame, routing, and multiplexing rules.
- The CRC applies only to onboard-profile flash sectors, never to wire frames; HID++ frames carry no checksum.
